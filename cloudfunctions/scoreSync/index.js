...
async function updateScore(data, operatorId) {
  ...
  // 验证操作者权限（检查是否为房间中的玩家）
  const player = room.players.find(p => p.playerId === operatorId);
  if (!player) {
    return {
      success: false,
      message: '无权限操作'
  ...
  let newLevel = { ...room.currentScore };

  switch (actionType) {
    case 'score':
      // 加分操作
      newScore[team] += points;
      // 根据分数计算等级（这里简化处理，实际需要根据掼蛋规则）
      newLevel[team] = calculateLevel(newScore[team]);
      break;
      
    case 'level_up':
      // 升级操作（如打过A）
      if (team === 'red') {
        newLevel.redAAttempts += 1;
      } else {
        newLevel.blueAAttempts += 1;
      }
      break;
  ...
  const scoreRecord = {
    ...
    operatorName: player.playerName
  };
...
async function resetScore(data, operatorId) {
  ...
  // 验证操作者权限（检查是否为房主）
  const isOwner = room.players.find(p => p.playerId === operatorId && p.isOwner);
  if (!isOwner) {
    return {
      success: false,
      message: '只有房主可以重置比分'
