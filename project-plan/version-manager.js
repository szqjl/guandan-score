...
class VersionManager {
  ...
  createVersion(version, description, features = [], changelog = []) {
    ...
    const versionInfo = {
      ...
      changelog: changelog,
      status: version.includes('stable')
        ? 'stable'
    ...
    console.log(
      `✅ 版本创建完成: ${version} (备份了 ${backupCount} 个项目文件)`
    )
    return true
  }
...
function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log('使用方法:')
      console.log(
        '  node version-manager.js create <版本号> [描述]     - 创建新版本（含完整备份）'
      )
      console.log(
        '  node version-manager.js complete <版本号>           - 完善现有版本备份'
      )
      console.log(
        '  node version-manager.js complete-all               - 完善所有版本备份'
      )
      console.log(
        '  node version-manager.js list                       - 列出所有版本'
      )
      console.log(
      ...
      console.log('示例:')
      console.log(
        '  node version-manager.js create v1.0.0-stable "基础核心功能版"'
      )
