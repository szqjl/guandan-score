...
Page({
  data: {
    ...
    // 新增过A制相关状态
    aAttempts: {
      red: 0, // 东西方队尝试过A的次数(0: 未尝试, 1:A¹, 2:A², 3:A³)
      blue: 0, // 南北方队尝试过A的次数
    },
    // 记录是否首次到达A阶段
    firstTimeReachA: {
      red: false, // 东西方队是否首次到达A阶段
      blue: false, // 南北方队是否首次到达A阶段
    },
    // 游戏是否已开始
    gameStarted: false,
    // 游戏是否已结束
  ...
  checkBashuWarning() {
    ...
    if (redInA && !blueInA) {
      ...
      // 如果T3追不上，但双上能追上，就预警
      if (
        blueLevel + maxPossibleIncrease < redLevel &&
        blueLevel + maxPossibleIncreaseDouble >= redLevel
      ) {
      ...
      // 如果T3追不上，但双上能追上，就预警
      if (
        redLevel + maxPossibleIncrease < blueLevel &&
        redLevel + maxPossibleIncreaseDouble >= blueLevel
      ) {
    ...
    // 如果T3追不上，但双上能追上，就预警
    if (
      levelGap > maxPossibleIncrease &&
      levelGap <= maxPossibleIncreaseDouble
    ) {
  ...
  onLevelUpgrade(e = {}) {
    ...
    if (buttonTeam && buttonType) {
      const buttonKey =
        buttonType === '3' ? 'double' : buttonType === '2' ? 'two' : 'one'
      this.setData({
    ...
    const { team: selectedTeam = 'red', value: levelValue = 'increase' } =
    ...
    // 多人模式：上传计分数据到云端
    if (this.data.isMultiMode) {
      this.uploadScoreData(selectedTeam, scoreIncrement, 'score')
    }
    ...
    if (this.data.rule === 'by-A') {
      ...
      if (currentLevel >= 14) {
        ...
        if (scoreIncrement >= 2) {
          const attemptText =
            attempt === 0 ? '第1轮' : attempt === 1 ? '第2轮' : '第3轮'
          wx.showModal({
            title: '本局结束',
            content: `${
              currentTeam === 'red' ? '恭喜红方' : '恭喜蓝方'
            }${attemptText}过A成功，本局结束！`,
            showCancel: false,
            success: (res) => {
              if (res.confirm) {
                this.setData({
          ...
          if (attempt < 3) {
            ...
            const attemptText =
              nextAttempt === 1
                ...
                ? '第2次'
                : '第3次'
            const teamName = currentTeam === 'red' ? '东西方队' : '南北方队'

            this.setData({
              [`aAttempts.${currentTeam}`]: nextAttempt,
              [`levelTexts.${currentTeam}`]:
                this.getFormattedAAttempt(nextAttempt),
          ...
          if (otherAttempt < 3) {
            ...
            const attemptText =
              nextAttempt === 1
                ...
                ? '第2次'
                : '第3次'
            const otherTeamName = otherTeam === 'red' ? '东西方队' : '南北方队'

            this.setData({
              [`aAttempts.${otherTeam}`]: nextAttempt,
              [`levelTexts.${otherTeam}`]:
                this.getFormattedAAttempt(nextAttempt),
      ...
      // 先检查是否达到最大把数(把数制)
      if (
        this.data.rule === 'by-rounds' &&
        nextRounds === this.data.maxRounds
      ) {
        ...
        if (uncatchableResult.isUncatchable) {
          wx.showModal({
            title: '追不上了',
            content: `${
              uncatchableResult.leadingTeam === 'red' ? '红方' : '蓝方'
            }领先优势过大,即使剩余把数全胜也无法追上!`,
            confirmText: '是',
            cancelText: '否',
            success: (res) => {
              if (res.confirm) {
  ...
  onRuleChange(e) {
    ...
    if (
      newRule === 'by-rounds' &&
      currentRedText.startsWith('A') &&
      !currentRedText.startsWith('A2') &&
      !currentRedText.startsWith('A3')
    ) {
      // 检查当前是否有A的尝试
      ...
      this.setData({
        'levelTexts.red':
          redScore === 14
            ? this.getFormattedAAttempt(this.data.aAttempts.red)
            : redScore === 11
            ? 'J'
            : redScore === 12
            ? 'Q'
        ...
        'levelTexts.blue':
          blueScore === 14
            ? this.getFormattedAAttempt(this.data.aAttempts.blue)
            : blueScore === 11
            ? 'J'
            : blueScore === 12
            ? 'Q'
  ...
  updateHistoryScore(team, newScore, index) {
    ...
    for (let i = index + 1; i < currentHistory.length; i++) {
      ...
      // 获取原始当前回合与前一回合的差值
      const originalPrevScore = this.textToLevel(
        this.data.historyScores[i - 1][team]
      )
      const originalCurrScore = this.textToLevel(
        this.data.historyScores[i][team]
      )
  ...
  undoLastStep() {
    ...
    if (
      this.data.levelTexts.red === 'E' ||
      this.data.levelTexts.blue === 'E' ||
      this.data.gameEnded
    ) {
      wx.showToast({
        title: '游戏已结束,不允许撤销',
        ...
        if (res.confirm) {
          ...
          if (savedGameData) {
            ...
            this.setData({
              ...
              gameEnded:
                savedGameData.levelTexts &&
                (savedGameData.levelTexts.red === 'E' ||
                  savedGameData.levelTexts.blue === 'E'),
            })
            wx.showToast({
              title: '游戏已恢复',
              icon: 'success',
  ...
  saveGameToHistory() {
    try {
      ...
      const hasGameData =
        this.data.historyScores.length > 0 || this.data.rounds > 0
      if (!hasGameData) {
        return
      }
  ...
  initRealTimeSync() {
    ...
    wx.onNetworkStatusChange((res) => {
      ...
      if (res.isConnected && this.data.isMultiMode) {
        // 网络恢复时重新开始同步
        this.startSync()
  ...
  // 开始实时同步
  startSync() {
    if (!this.data.isMultiMode || !this.data.roomId || !this.data.isOnline) {
      return
    }
  ...
  syncGameData() {
    ...
    wx.cloud
      ...
      .then((res) => {
        if (res.result.success) {
          const { currentScore, scoreHistory } = res.result.data

          // 更新本地数据
          this.setData({
            levelTexts: {
              red: currentScore.redLevel || '2',
              blue: currentScore.blueLevel || '2',
            },
            aAttempts: {
              red: currentScore.redAAttempts || 0,
              blue: currentScore.blueAAttempts || 0,
            },
  ...
  // 上传计分数据到云端
  uploadScoreData(team, points, actionType = 'score') {
    if (!this.data.isMultiMode || !this.data.roomId) {
      return
    }
